 ...
 */
/**
 * The display screen functionality of the plugin.
 *
 * Defines the Screen Functions
 *
 * @package    Wp_Bp_Follow
 * @subpackage Wp_Bp_Follow/includes
 ...
 */
function wp_bp_follow_screen_following() {
    do_action('wp_bp_follow_screen_following');

    // ignore the template referenced here
    // 'members/single/following' is for older themes already using this template
    //
	// view wp_bp_follow_load_template_filter() for more info
 ...
 * @uses bp_core_load_template() Loads a template file.
 */
function wp_bp_follow_screen_followers() {
    global $bp;

    do_action('wp_bp_follow_screen_followers');
 ...
 * @uses bp_core_load_template() Loads a template file.
 */
function wp_bp_follow_screen_activity_following() {
    wp_redirect(home_url('following-posts'));
}

/** TEMPLATE LOADER *********************************************** */

/**
 * WP BP Follow template loader.
 *
 * This function sets up WP BP Follow to use custom templates.
 *
 ...
 * @since 1.0
 */
function wp_bp_follow_load_template_filter($found_template, $templates) {
    global $bp;
    ...
    if (empty($found_template)) {
        ...
        bp_register_template_stack('wp_bp_follow_get_template_directory', 14);
        ...
        // can disable with the 'wp_bp_follow_allow_ajax_on_follow_pages' filter
        if (apply_filters('wp_bp_follow_allow_ajax_on_follow_pages', true)) {
            // add the "Order by" dropdown filter
            add_action('bp_member_plugin_options_nav', 'wp_bp_follow_add_members_dropdown_filter');

            // add ability to use AJAX
            add_action('bp_after_member_plugin_template', 'wp_bp_follow_add_ajax_to_members_loop');
        }
    ...
    return apply_filters('wp_bp_follow_load_template_filter', $found_template);
}

add_filter('bp_located_template', 'wp_bp_follow_load_template_filter', 10, 2);
 ...
 * @since 1.0.0
 *
 * @see wp_bp_follow_load_template_filter()
 */
function wp_bp_follow_add_members_dropdown_filter() {
    ?>
    ...
    <?php // the ID for this is important as AJAX relies on it!  ?>
        <label for="members-<?php echo bp_current_action(); ?>-orderby"><?php _e('Order By:', WPBP_FOLLOW_DOMAIN ); ?></label>
        <select id="members-<?php echo bp_current_action(); ?>-orderby">
            <option value="active"><?php _e('Last Active', WPBP_FOLLOW_DOMAIN ); ?></option>
            <option value="newest"><?php _e('Newest Registered', WPBP_FOLLOW_DOMAIN ); ?></option>

            <?php if (bp_is_active('xprofile')) : ?>
                <option value="alphabetical"><?php _e('Alphabetical', WPBP_FOLLOW_DOMAIN ); ?></option>
            <?php endif; ?>
 ...
 * @return string
 */
function wp_bp_follow_get_template_directory() {
    return apply_filters('wp_bp_follow_get_template_directory', WP_BP_FOLLOW_DIR . '/includes/templates');
}
 ...
 * @see wp_bp_follow_load_template_filter()
 */
function wp_bp_follow_add_ajax_to_members_loop() {
    ?>
