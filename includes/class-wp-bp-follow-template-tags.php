 ...
 *
 * Defines the template functions
 *
 * @package    Wp_Bp_Follow
 ...
 *
 * @param mixed $args See wp_bp_follow_get_add_follow_button() for full arguments.
 * @uses wp_bp_follow_get_add_follow_button() Returns the follow / unfollow button
 * @author Wbcom Designs <admin@wbcomdesigns.com>
 * @since 1.0.0
 */
function wp_bp_follow_add_follow_button($args = '') {
    echo wp_bp_follow_get_add_follow_button($args);
}
 ...
 */
function wp_bp_follow_get_add_follow_button($args = '') {
    ...
    if (!empty($members_template->in_the_loop) && $r['follower_id'] == bp_loggedin_user_id() && $r['leader_id'] == bp_get_member_user_id()) {
        ...
        $is_following = wp_bp_follow_is_following(array(
    ...
    if ($is_following) {
        ...
        $action = 'stop';
        $class = 'unfollow';
        $link_text = sprintf(_x('Unfollow', 'Button', WPBP_FOLLOW_DOMAIN ), apply_filters('wp_bp_follow_leader_name', bp_get_user_firstname($leader_fullname), $r['leader_id']));
        ...
        $action = 'start';
        $class = 'follow';
        $link_text = sprintf(_x('Follow', 'Button', WPBP_FOLLOW_DOMAIN ), apply_filters('wp_bp_follow_leader_name', bp_get_user_firstname($leader_fullname), $r['leader_id']));
    ...
    return bp_get_button(apply_filters('wp_bp_follow_get_add_follow_button', $button, $r['leader_id'], $r['follower_id']));
}
 ...
 */
function wp_bp_following_ids($args = '') {
    echo wp_bp_get_following_ids($args);
}
 ...
 */
function wp_bp_get_following_ids($args = '') {

    $r = wp_parse_args($args, array(
        'user_id' => bp_displayed_user_id()
    ));

    $ids = implode(',', (array) wp_bp_follow_get_following(array('user_id' => $r['user_id'])));

    $ids = empty($ids) ? 0 : $ids;

    return apply_filters('wp_bp_get_following_ids', $ids, $r['user_id']);
}
 ...
 */
function wp_bp_follower_ids($args = '') {
    echo wp_bp_get_follower_ids($args);
}
 ...
 * @return Mixed Comma-seperated string of user IDs on success. Integer zero on failure.
 */
function wp_bp_get_follower_ids($args = '') {

    $r = wp_parse_args($args, array(
        'user_id' => bp_displayed_user_id()
    ));

    $ids = implode(',', (array) wp_bp_follow_get_followers(array('user_id' => $r['user_id'])));

    $ids = empty($ids) ? 0 : $ids;

    return apply_filters('wp_bp_get_follower_ids', $ids, $r['user_id']);
}
